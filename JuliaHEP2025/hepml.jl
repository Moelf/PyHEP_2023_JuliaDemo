...
begin
	using Random, LinearAlgebra, Serialization, Static
	using PlutoUI
	using OhMyThreads
	using Lux, Reactant, MLUtils, Optimisers
	using LorentzVectorHEP
	using DifferentiationInterface; const DI = DifferentiationInterface
	using CairoMakie
	...
	set_theme!(theme_latexfonts())  # makie
end
...
## What is it?
Given some code we could just analytically figure out its derivative and write down that function, but it would be better if we could have the compiler do it for us.
"""
...
md"""
Let's declare the derivatives of some functions...
"""

# ╔═╡ c0e1ad9f-9d72-498b-8ad3-6326624f6304
∂(::typeof(square)) = x -> 2*x

# ╔═╡ 75311a58-587b-402a-aa3d-0644bcaf4145
∂(::typeof(sin)) = cos

# ╔═╡ 6fff31a4-f312-4bc7-b5c2-6bf48c1f17dc
md"""
The crucial step is to define the chain rule.  We do this using `ComposedFunction` which in Julia is what you get when you do `f ∘ g`.
"""

# ╔═╡ 0ed4ba0d-8136-4369-87a3-fee752c93cc2
∂(f::ComposedFunction) = x -> begin
	∂(f.outer)(f.inner(x)) * ∂(f.inner)(x)
end

# ╔═╡ c30c6ab3-8633-4546-837a-62da4ebfbe41
∂(sin ∘ square)(√(π)) ≈ 2*√(π)*(-1)

# ╔═╡ 32e6c08b-e256-466e-aa7f-82228d247983
md"""
Note that the two-function chain rule is enough for us to now differentiate _any_ composition of functions for which we have defined derivatives because of recursion.
"""

# ╔═╡ d0ca186a-4955-4a50-962e-da460c052f5e
∂(square ∘ sin ∘ square)(√(π/4)) ≈ 4*√(π/4)*sin(π/4)*cos(π/4)

# ╔═╡ 4b3dbb5e-6ace-42a3-8e77-5d8907320bc0
md"""
One approach is to create dedicated data structures and then define this kind of stuff for them.  This gives us lots of opportunities to optimize the hell out of it but it is not very general: you are basically programming in all the derivatives by hand.  This is roughly what `pytorch` and `tensorflow` do.

Another approach is to try to get your compiler to do this for _everything in the language_.  This is really hard, but very powerful.  This latter approach is more common in Julia.
"""

# ╔═╡ 07e47d9b-2c55-4cf9-8da9-14d7ce285126
md"""
...
```
```math
h = f_l \circ f_{l-1} \circ \cdots \circ f_1
```
the Jacobian is
```math
...
Each is more efficint in different cases
- **Forward:** best for ``m \gg n`` (because we have to do a separate forward pass for each argument!)
...
md"""
## The Julia AutoDiff Ecosystem
Julia has a bewildering variety of different autodiff packages.  Some might say that autodiff is Julia's "hello world!"

### The Easy Answer: [ForwardDiff.jl](https://github.com/JuliaDiff/ForwardDiff.jl)
ForwardDiff is fast, relatively simple and doesn't involve any fancy compiler shenanigans.  The disadvantage is that it's only forward mode.

!!! warning "Forward Mode is Slow for Deep Learning"
	Unfortunately forward mode autodiff isn't really usable for deep learning in most applications, so you won't see it talked about much in regard to ML.

### The Emerging Standard: [Enzyme.jl](https://github.com/EnzymeAD/Enzyme.jl)
Enzyme supports both forward and reverse mode and can differentiate almost anything.  In most cases it will "just work", but its internals are rather opaque for the overwhelming majority of users.

### Old Reliable for Reverse Mode: [Zygote.jl](https://github.com/FluxML/Zygote.jl)
Zygote has been around for a while and will work ok for most use cases, including deep learning.

### Tasty Desserts: [Mooncake.jl](https://github.com/chalk-lab/Mooncake.jl)
Mooncake is a newer pure Julia reverse mode autodiff akin to Zygote.
"""

# ╔═╡ 56e5a9a4-0026-47c1-9a48-7513877017b2
md"""
## [DifferentiationInterface.jl](https://github.com/JuliaDiff/DifferentiationInterface.jl)
Julia arguably has _too many_ ways to do autodiff.  It is crucial for many use cases to be able to easily swap-out backends
"""

# ╔═╡ bf695a72-df00-4f92-8f48-b6942ab7451c
LocalResource("autodiff-julia.png")

# ╔═╡ ee0b60d1-9b99-48ed-be1e-7fae89586c69
md"""
### Basic Usage Examples
"""

# ╔═╡ 7c2e9b7c-e117-4e76-a82f-2e31d0a2b0a1
derivative(sin, AutoForwardDiff(), π)

# ╔═╡ c41e648a-f5d1-4b54-9888-5687914fe82a
gradient(AutoForwardDiff(), Float64[1,2,3]) do x
	sum(abs2, x)
end

# ╔═╡ 713cfc2f-a6ce-4d36-bbe5-0d8dfd750174
jacobian(AutoForwardDiff(), Float64[1,2,3]) do x
	[x[1]*x[2], x[2]*x[3], x[1]^2]
end

# ╔═╡ cf5b746b-b544-4045-be60-f9b56140591a
md"""
The types for selecting the autodiff method are provided by `AdTypes.jl` (re-exported from DI).
- `AutoForwardDiff()`
- `AutoEnzyme(mode=Enzyme.Forward)`, `AutoEnzyme(mode=Enzyme.Reverse)`
- `AutoZygote()`
- `AutoMooncake()`
- `AutoFiniteDiff()` this is finite differences, not true autodiff, but it can be very useful for validation and comparison!

The ease with which one can swap between entirely different autodiff methods is a significant advantage of the Julia approach to autodiff.
"""

# ╔═╡ 3ec65f97-0792-4f3d-9d7f-0b543c3c34d0
md"""
### ML Use Cases
For most ML use cases all you will need is the `AdTypes.jl` argument to tell it how to differentiate (but it's good to understand how things work).
"""
...
Julia has a number of deep learning frameworks and if you really want you could wrap a Python one with varying cost.

[Flux.jl](https://github.com/FluxML/Flux.jl) has been around for a while.
...
begin
	device = cpu_device()  # normally we'll use reactant, but this is for demonstration purposes
	rng = Xoshiro(999)
	
	mlp = Chain(
		Dense(8=>32),
		Dense(32=>32, relu), Dense(32=>32, relu),
		Dense(32=>1),
	)

	# construct parameters and state and move to the device
	(θ, ψ) = Lux.setup(rng, mlp) |> device
...
begin  # training
	...
	s = Training.TrainState(mlp, θ, ψ, Adam(0.001f0))

	y = randn(Float32, 1, 1024)

	# we explicitly loop over the data and call train steps individually
	for j ∈ 1:10
		# note we are providing the autodiff backend here!
		(grads, ℓ, stats, s) = Training.single_train_step!(AutoZygote(),
			MSELoss(), (x, y), s,
		)
	end
end
...
md"""
# Full Lux Example
Here we are going to do some rather silly and contrived event tagging, for the sake of demonstrating model building with Lux.

I've prepared some data consisting of the hardest jet for each event.  Here's a structure for storing the data of each jet:
"""

# ╔═╡ d9b66790-0dde-425f-b100-11d0f358cd0a
struct Jet
	class::Bool  # whether the event has a Z (pythia)
	n_particles::Int  # this isn't realistic
	total_charge::Float64  # neither is this
	mass::Float64
	pT::Float64
	η::Float64
	C1::Float64  # energy correlation jet substructure coeff
...
begin
	...
	@eval Main import .$modu: Jet
end

# ╔═╡ 33d0d1df-eb88-424d-9940-c42c999082a5
begin  # make a data iterator with MLUtils.jl
	struct JetDataset
		jets::Vector{Jet}
	end
	...
	function getjetclass(jds::JetDataset, idxs)
    	...
    	for (k, idx) ∈ enumerate(idxs)
      		o[1,k] = !jds.jets[idx].class
        	o[2,k] = jds.jets[idx].class
    	end
    	o
	end
	...
	# we have a different backend depending on whether we use reactant
	guessdiffbackend(dev::AbstractDevice) = AutoZygote()
	guessdiffbackend(dev::ReactantDevice) = AutoEnzyme()
end

# ╔═╡ 85c48ca5-7fc9-42a8-b7dd-36b51732f5e3
md"""
Now we make a function that creates the model and trains it...
"""

# ╔═╡ 931f7c56-8458-4d15-b194-b4b3783a9d65
function train(rng::AbstractRNG, traindat;
        nepochs::Integer=50,
		width::Integer=64,
        device::AbstractDevice=reactant_device(),
        batchsize::Integer=512,
        opt::AbstractRule=Adam(0.001f0),
        diffbackend::AbstractADType=guessdiffbackend(device),
    )
	# this put our data into an appropriate form and move it to the device
    data = DataLoader(traindat; batchsize, shuffle=true, partial=false) |> device

	# this is the actual model
	# note that the final output is softmax because it's a classifier
    mlp = Chain(
        BatchNorm(7),  # rescale features, in real life you'd do this globally instead of per batch
		Dense(7=>width),
        Dense(width=>width, relu), Dense(width=>width, relu), Dense(width=>2),
        WrappedFunction(softmax),
    )

    (θ, ψ) = Lux.setup(rng, mlp) |> device

    s = Training.TrainState(mlp, θ, ψ, opt)
    ...
    for j ∈ 1:nepochs
        for batch ∈ data
            (_, ℓ, stats, s) = Training.single_train_step!(diffbackend,
                ...
                return_gradients=False(),  # not needed for training
            )
        end
		@info("epoch :$j ; loss: $(Float32(ℓ))")
    end

    @info("final loss: $(Float32(ℓ))")

	# we use this wrapper to return the model wrapped as a single object
	# let's move it to CPU for now
	(θo, ψo) = (s.parameters, LuxCore.testmode(s.states)) |> cpu_device()
...
function validate(mdl::StatefulLuxLayer, testdat;
		threshold::Real=0.5,
	)
	(X, y) = getobs(testdat, :)

	clsfy = ζ -> ζ[2] > threshold

    ŷ = map(clsfy, eachcol(mdl(X)))  # predicted to bools
    y = map(clsfy, eachcol(y))  # test data to bools

    ys = zip(y, ŷ)

    tp = count(ys) do (ζ, ζ̂)  # true positive
        ζ && ζ̂
    end
    fn = count(ys) do (ζ, ζ̂)  # false negative
        ζ && !ζ̂
    end
    fp = count(ys) do (ζ, ζ̂)  # false positive
        !ζ && ζ̂
    end
    tn = count(ys) do (ζ, ζ̂)  # true negative
        !ζ && !ζ̂
    end
...
function main(rng::AbstractRNG=Xoshiro(999);
        device::AbstractDevice=reactant_device(),
        kw...,
    )
    jds = JetDataset(loadjets())

	# this helper function from MLUtils will split the data for us
    (traindat, testdat) = splitobs(rng, jds, at=0.8, shuffle=true)
	
    mdl = train(rng, traindat; device, kw...)
    o = validate(mdl, testdat)
    @info("test results: ", o)
    (mdl, testdat, o)
end

# ╔═╡ 9bb42035-a587-419b-bb02-4228c7d5421f
begin
	(mdl, testdat, test_results) = main()
	test_results
end
...
function roc(mdl::StatefulLuxLayer, testdat; δthreshold::Real=0.01)
	threshs = 0.0:δthreshold:1.0
	rs = tmap(threshs) do thresh
		o = validate(mdl, testdat; threshold=thresh)
		tpr = o.tp / (o.tp + o.fn)
		fpr = o.fp / (o.fp + o.tn)
		(tpr, fpr)
	end
...
# ╔═╡ 47661ebc-b7fa-4c16-9c3d-6ef1158377aa
(fpr, tpr)  = roc(mdl, testdat)

# ╔═╡ e6a53eae-52f7-4d65-b3fc-640c858946c5
md"""
We have wound up with an ok-ish classifier (considering we put in so little effort!):
"""

# ╔═╡ e7f3b8e1-a999-49d0-9e8e-3218211bcb5b
let
...
git-tree-sha1 = "9b9bf18055e365fc4d120516ed286dedc308f23d"
...
deps = ["LinearAlgebra"]
git-tree-sha1 = "c51f5ff7ef7f42defbf0e36ebd2932c5e34a9fcc"
uuid = "f612022c-142a-473f-8cfd-a09cf3793c6c"
version = "0.1.8"
weakdeps = ["StaticArrays"]

    [deps.LorentzVectorHEP.extensions]
...
git-tree-sha1 = "81460ab0c7ec78dc43ed4110ec3f7ded4eb1bfc7"
...
git-tree-sha1 = "c7ec05200b75a7da85c351cdd0a9082860f90294"
...
git-tree-sha1 = "120feaf6a97738e3a63902644a0afb3b69cc7b98"
uuid = "a3311ec8-5e00-46d5-b541-4f83e724a433"
version = "0.1.15"

[[deps.Reactant_jll]]
deps = ["Artifacts", "CUDA_Driver_jll", "JLLWrappers", "LazyArtifacts", "Libdl", "TOML"]
git-tree-sha1 = "5772aff9d8b8229e1173ddea317a95e3e8a61856"
